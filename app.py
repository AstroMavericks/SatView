...
def send_confirmation_email(user_id, latitude, longitude, notification_time):
    ...
    We have received your satellite pass notification request for:
    Latitude: {latitude}
    Longitude: {longitude}
    Notification time: {notification_time} minutes before pass
    
    We will notify you when the satellite passes over your location.
    """
    ...
    try:
        ...
        print(f"Confirmation email sent successfully to {user_id}")
...
def home():
    if request.method == 'POST':
        try:
            ...
            # Send confirmation email with input details
            send_confirmation_email(user_id, latitude, longitude, notification_time)
            ...
            map_data = {
                'latitude': latitude,
                'longitude': longitude,
                'center_pixel': [latitude, longitude],
                'surrounding_pixels': get_surrounding_pixels(latitude, longitude)
            }
...
def map():
    ...
    # Check if lat and lon are None
    if lat is None or lon is None:
        lat, lon = 9.6684, 76.8246  # Default to a location if not provided

    print(f"Latitude: {lat}, Longitude: {lon}")  # Debugging statement
    
    map_data = {
        'latitude': lat,
        'longitude': lon,
        'center_pixel': [lat, lon],
        'surrounding_pixels': get_surrounding_pixels(lat, lon)
    }
    return render_template('map.html', map_data=json.dumps(map_data))
...
def get_surrounding_pixels(lat, lon, offset=0.00027):
    pixels = []
    for i in range(-1, 2):
        for j in range(-1, 2):
            pixels.append([lat + i * offset, lon + j * offset])
    return pixels
